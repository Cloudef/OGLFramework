...
#ifndef DL_WINDOW_DISABLE_LOG
   #define DL_WINDOW_DISABLE_LOG 1
#endif

#if DL_WINDOW_DISABLE_LOG
   #define logOpen(x) NULL
   #define logRed()     ;
   #define logBlue()    ;
   #define logGreen()   ;
   #define logYellow()  ;
   #define logNormal()  ;
   #define logPuts(x,y)       puts(y)
   #define logPrint(x,y,...)  printf(y,...)
#else
   #include "logfile.h"
#endif
