...
glTexturePacker* glNewTexturePacker( void )
{
   ...
   tp = calloc( 1, sizeof( glTexturePacker ) );
   if(!tp)
      return( NULL );
...
static void glResetTexturePacker( glTexturePacker* tp )
{
   tp->textureCount = 0;
   tp->longestEdge  = 0;
   tp->totalArea    = 0;
...
static void glTexturePackerNewNode( glTexturePacker *tp, int x, int y, int wid, int hit )
{
   node_t *node;
...
static void validate( glTexturePacker *tp )
{
#ifdef DEBUG
   node_t *c;
...
static int mergeNodes( glTexturePacker *tp )
{
   node_t *prev;
   node_t *c;
   node_t *f = tp->freeList;
...
int glTexturePackerGetLocation( glTexturePacker* tp, int index, int *inX, int *inY, int *inW, int *inH )
{
   int ret  = 0;
   int x    = 0;
   int y    = 0;
...
int glTexturePackerPack( glTexturePacker *tp, int *inWidth, int *inHeight, int forcePowerOfTwo, int onePixelBorder )
{
   int width  = 0;
   int height = 0;
   int count;
   ...
   tp->debugCount = 0;
   glTexturePackerNewNode( tp, 0, 0, width, height );
         ...
         switch( edgeCount )
         {
            case 0:
               if ( t->longestEdge <= bestFit->width )
               {
                  ...
                  glTexturePackerNewNode( tp, bestFit->x, bestFit->y + hit, bestFit->width , bestFit->height - hit );
