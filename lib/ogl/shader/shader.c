...
static const glShaderType uniformTypes[] =
{
   /* glsl type */      /* GL type */
...
static char* getFile( const char *file )
{
   ...
   {
      fclose(f);
      logRed(); glPuts("Shader file too big."); logNormal();
      return( NULL );
   }
...
/* link shader program */
static int link_shader( glShader *shader )
{
...
/* process shader */
static int process_shader( glShader *shader )
{
   unsigned int i;
   ...
   {
      logRed(); glPuts("[SHADER] Failed to retieve uniform information"); logNormal();
      return( RETURN_FAIL );
   }

   shader->uniformCount = (unsigned int)uniformCount;
   shader->uniforms     = glCalloc( uniformCount, sizeof( glShaderUniform ) );
   if(!shader->uniforms)
   {
      logRed(); glPuts("[SHADER] Failed to alloc uniform list"); logNormal();
      return( RETURN_FAIL );
   }
   ...
   if(!buffer)
   {
      logRed(); glPuts("[SHADER] Failed to alloc uniform name buffer"); logNormal();
      return( RETURN_FAIL );
   }
...
glShader* glNewShader( const char *file )
{
   ...
   /* allocate shader */
   glSetAlloc( ALLOC_SHADER );
   shader = glCalloc( 1, sizeof( glShader ) );
   if(!shader)
   {
   ...
   { glFreeShader( shader ); return( NULL ); }
...
int glFreeShader( glShader *shader )
{
   ...
   if( shader->uniforms )
   {
      ...
      for(; i != shader->uniformCount; ++i)
         if(shader->uniforms[i].name) free( shader->uniforms[i].name );

      glFree( shader->uniforms,
      sizeof( glShaderUniform ) * shader->uniformCount );
   }
